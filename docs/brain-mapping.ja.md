...
PrimingEngineは6チャネルの並列記憶検索を実行し、各チャネルは異なる神経認知機能に対応する：

| チャネル | 機能 | 脳の対応 | トークンバジェット |
|---|---|---|---|
| A: 送信者プロファイル | 「誰が話しかけているか？」 | 紡錘状顔領域・側頭極（人物認識） | 500 |
| B: 直近の活動 | 「最近何があったか？」 | 海馬リプレイ（直近エピソードの再活性化） | 1300 |
| C: 関連知識 | 「これについて何を知っているか？」 | 意味記憶の検索（側頭葉皮質） | 1200 |
| D: スキルマッチング | 「これを処理できるか？」 | 手続き記憶の活性化（基底核） | 200 |
| E: 未完了タスク | 「何をすべきだったか？」 | 展望的記憶・意図モニタリング（吻側前頭前野） | 300 |
| F: エピソード | 「過去に似た経験は？」 | エピソード記憶の意味検索（海馬-皮質） | 500 |

チャネルDはマッチしたスキル/手続きの**名前のみ**を返却する（段階開示、最大5件）。全文はエージェントが`skill`ツールで必要時に読み込む。チャネルCは検索結果を信頼レベル（medium / untrusted）で分離し、外部由来の知識は別枠で注入する。チャネルBは統一アクティビティログ（`activity_log/`）と共有チャネル（`shared/channels/`）から直近の活動を取得する。heartbeat/cronでは、tool_use・tool_result・heartbeat_start/end・inbox_processing_start/end をノイズとして除外し、アクション可能な通信イベントのみを注入する。

これに加え、PrimingEngineは**直近アウトバウンド**（直近2時間のチャネル投稿・DM送信、最大3件）を収集し、システムプロンプトに注入する。これは「自分が最近何をしたか」の自己モニタリングに対応し、重複送信の抑制や行動の一貫性維持に寄与する。builderはActivityLoggerを直接読まず、PrimingEngineがプロンプト構築における唯一のアクティビティリーダー（海馬モデル）として機能する。
...
加えて、トリガー（`chat` / `inbox` / `heartbeat` / `cron` / `task`）に応じたセクション選択が行われる。heartbeatとcronはspecialty・emotion・a_reflectionを省略し、taskはidentity 3行とタスク記述のみの最小コンテキストで実行する。実行パスごとに「何を意識に上らせるか」を制御することで、認知負荷を最適化する。

#### 統一アクティビティログ

PrimingチャネルBの唯一のデータソースとして、`ActivityLogger` が全Animaインタラクションを `{anima_dir}/activity_log/{date}.jsonl` に時系列記録する。主なイベントタイプ：`message_received` / `message_sent`（旧 `dm_received` / `dm_sent` と後方互換）、`response_sent`、`channel_read` / `channel_post`、`human_notify`、`tool_use` / `tool_result`、`heartbeat_start` / `heartbeat_end` / `heartbeat_reflection`、`inbox_processing_start` / `inbox_processing_end`、`cron_executed`、`memory_write`、`error`、`issue_resolved`、`task_created` / `task_updated`。ストリーミング出力のクラッシュ耐性は `shortterm/streaming_journal_{session_type}.jsonl`（chat/heartbeat別）のWrite-Ahead Logで実現する。
...
| **日次固定化**（深夜cron） | NREM睡眠の徐波-紡錘波-リップルカスケード | Animaがツール経由で実行。episodes/ → knowledge/ の知識抽出・バリデーション。ConsolidationEngineは前処理（エピソード収集・issue_resolved収集）と後処理（RAGインデックス更新）を提供 |
...
| **完全忘却**（月次） | 閾値以下のシナプス消失 | アーカイブ→削除。手続きバージョンアーカイブは直近5版のみ保持 |
| **忘却耐性**（procedures, skills, knowledge） | 基底核の手続き記憶は忘却に強い | procedures: version >= 3 または protected: true、または utility-based 保護（失敗率低・使用頻度高）。knowledge: success_count >= 2。skills / shared_users は完全保護 |
...
AnimaWorksは4つの実行モードを定義しており、それぞれ異なるレベルの認知的自律性に対応する：

| モード | エグゼキュータ | 脳のアナロジー | 説明 |
|---|---|---|---|
| **S**（SDK） | Claude Agent SDK | 実行制御を伴う完全な皮質機能 | ネイティブのClaudeツール使用とセッション継続。最も自律的なモード。前頭前野の実行機能が完全に機能する覚醒脳に対応 |
| **C**（Codex） | Codex SDK（Codex CLI） | Sと同様の完全な皮質機能 | OpenAIモデルをCodex CLI経由で実行。MCP統合・サンドボックス分離。Sと同様にツール使用とセッション継続をネイティブに扱う |
...
モード名（S/C/A/B）は、以前のA1/A2/B規則に代わるものである。モデル名からワイルドカードパターンマッチで自動判定され、Anima単位の明示的オーバーライドも可能。SとCはMCPスタイルのツールアクセスを共有する。
