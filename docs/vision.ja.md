...
> 更新日: 2026-03-05
> 関連: [brain-mapping.ja.md](brain-mapping.ja.md), [memory.ja.md](memory.ja.md)
...
AnimaWorksでは、ひとつのモデルが全てを担わない。Claude OpusやSonnetのような強力なモデルはエンジニアやマネージャーとして複雑な推論を担い、GPT-4oやGeminiは情報収集や汎用タスクをこなし、vLLMやOllamaなどのローカルモデルはログ監視や定型業務をコツコツ処理する。
...
- **想起**: メッセージを受け取ると、送信者プロファイル・直近活動（統一アクティビティログ）・関連知識・スキルマッチ・未完了タスク・エピソードなど6チャネルで関連記憶が自動想起される（プライミング）。直近アウトバウンド（自分が最近送ったもの）も注入され、重複送信の抑制に寄与する。スキルは段階開示（名前のみを想起、必要時に全文を読み込む）で認知負荷を抑える
- **学習**: 経験をエピソードとして記録し、繰り返し参照された知識は強化される。解決済み問題から手続き記憶が自動生成される
- **忘却**: 使われない記憶は徐々に薄れ、やがて消える。3段階（シナプスダウンスケーリング→ニューロン新生再編→完全忘却）で能動的に整理する
- **固定化**: 日々の経験が、汎用的な知識へと昇華される（エピソード→意味記憶）。手続き記憶は失敗からの再固定化（リコンソリデーション）で改善される
...
- **自律**: 各Animaはハートビートとcronで自ら動き、自分の理念で判断する。重い実行は計画と分離し、専用のタスク実行パスで処理する。Anima間メッセージ（Inbox）は即時応答パスで、ハートビートを待たずに返信できる
