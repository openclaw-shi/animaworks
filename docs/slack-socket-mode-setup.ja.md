...
3. **call_human スレッド返信**: メッセージがスレッド返信かつ `route_thread_reply` でマッピング済みの場合、元の通知を送ったAnimaのinboxにルーティング（`core/notification/reply_routing.py`）
4. **ルーティング解決**:
   - **Socket Mode Per-Anima Bot**: そのBotの全メッセージを対応Animaに直接配送
   - **Socket Mode 共有Bot**: `anima_mapping.get(channel_id) or default_anima`
   - **Webhook**: `app_id_mapping.get(api_app_id)` でAnimaを取得 → なければ `anima_mapping.get(channel_id) or default_anima`
5. **intent 検出**: DM または Bot メンション時は `intent="question"` を付与（`_detect_slack_intent`）
6. `Messenger.receive_external()` が `~/.animaworks/shared/inbox/{anima_name}/{msg_id}.json` にメッセージ配置
7. Animaが次のrunサイクル（heartbeat/cron/手動）でinboxを処理

## 関連ファイル

| ファイル | 役割 |
|----------|------|
| `server/slack_socket.py` | SlackSocketModeManager実装（Socket Mode、Per-Anima / 共有Bot両対応） |
| `server/app.py:174-183, 310-312` | lifespan内での起動/停止 |
| `server/routes/webhooks.py:78-183` | Webhook方式のエンドポイント（`/api/webhooks/slack/events`、mode=webhook時、app_id_mapping・default_anima対応） |
| `core/messenger.py` | `receive_external()` — inbox配置 |
| `core/notification/reply_routing.py` | call_human スレッド返信のAnimaルーティング |
| `core/config/models.py:181-189` | `ExternalMessagingChannelConfig` / `ExternalMessagingConfig` モデル |
