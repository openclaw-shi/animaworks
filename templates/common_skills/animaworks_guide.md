...
description: >-
  AnimaWorksフレームワークの仕組みと使い方を理解するための共通ナレッジ。
  「このシステムの使い方は？」「AnimaWorksとは？」等の質問があった場合に参照する。
  「AnimaWorks」「システムの使い方」「動作原理」「仕組み」
---
...
- AnimaWorks のソースコード（`~/dev/animaworks/`）は開発用であり、通常業務ではアクセスしない
