...
ハートビートは、Digital Person が定期的に自動起動し、状況を確認する仕組み。
...
`heartbeat.md` は各 Person の設定ファイルで、活動時間・チェック項目を定義する。
ハートビートの実行間隔は30分固定（システム管理）であり、Person が変更することはできない。
ファイルパス: `~/.animaworks/persons/{name}/heartbeat.md`
...
- カスタマイズ可能: Person の役割に合わせて項目を追加・変更してよい
...
heartbeat.md をファイルシステム上で更新すると、Person が自動的にスケジュールをリロードする。
サーバーの再起動は不要（MAY skip restart）。内部的に `reload_person_schedule()` が呼ばれ、APScheduler のジョブが再登録される。
...
ファイルパス: `~/.animaworks/persons/{name}/cron.md`
...
WebSocket 経由で `person.cron` イベントとしてブロードキャストもされる。
...
Person 自身が cron.md を書き換えた場合も即座に反映される（self-modify パターン）。

リロード時の動作:
1. 該当 Person の既存 cron ジョブを全て削除
