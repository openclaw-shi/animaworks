...
class TestGenerateExpressionEndpoint:
    """Tests for POST /persons/{name}/assets/generate-expression."""
    ...
    async def test_person_not_found(self, tmp_path):
        ...
        async with AsyncClient(transport=transport, base_url="http://test") as client:
            resp = await client.post(
                "/api/persons/nobody/assets/generate-expression",
    ...
    async def test_no_reference_image(self, tmp_path):
        ...
        async with AsyncClient(transport=transport, base_url="http://test") as client:
            resp = await client.post(
                "/api/persons/alice/assets/generate-expression",
                json={"expression": "smile"},
            )
        assert resp.status_code == 404
    ...
    async def test_invalid_expression_rejected(self, tmp_path):
        ...
        (person_dir / "assets" / "avatar_fullbody.png").write_bytes(b"fake")
        app = _make_test_app(persons_dir)
        transport = ASGITransport(app=app)
        async with AsyncClient(transport=transport, base_url="http://test") as client:
            resp = await client.post(
